...
class GVValuationForm {
    ...
    public function inject_critical_css() {
        echo '<style id="gvval-critical">body .gvval-wrapper{display:block;position:relative;width:min(100%,var(--gv-wrapper-width,760px));max-width:var(--gv-wrapper-width,760px);min-width:min(100%,var(--gv-wrapper-width,760px));min-height:calc(var(--gv-step-min-h,560px)+140px);}body .gvval-wrapper .gvval-step{display:none;}body .gvval-wrapper .gvval-step.gvval-active{display:block;}body .elementor-popup-modal .dialog-widget-content,body .elementor-lightbox .dialog-widget-content{display:flex!important;justify-content:center;align-items:center;padding:0!important;}body .elementor-popup-modal .dialog-message,body .elementor-lightbox .dialog-message{display:flex!important;justify-content:center;align-items:center;width:100%!important;max-width:none!important;padding:0!important;}</style>';
    }
    ...
    protected function render_analytics_tab() {
        ...
        echo '</div>';
        echo '<style>
        .gvval-admin .gvval-analytics-header{display:flex;align-items:center;gap:10px;margin:12px 0 12px;}
        .gvval-admin .gvval-analytics-note{font-size:13px;color:#4b5563;}
        .gvval-admin .gvval-analytics-table-wrap{max-width:420px;margin:0;padding:0;}
        .gvval-admin .gvval-analytics-table{margin-top:0;border-radius:12px;overflow:hidden;box-shadow:0 10px 24px rgba(15,23,42,0.08);width:100%;table-layout:fixed;}
        .gvval-admin .gvval-analytics-table col.gvval-analytics-step{width:38%;}
        .gvval-admin .gvval-analytics-table col.gvval-analytics-metric{width:20.5%;}
        .gvval-admin .gvval-analytics-table th,
        .gvval-admin .gvval-analytics-table td{padding:4px 8px;text-align:center;font-size:12px;}
    ...
    protected function merge_submission_data( $session_id, $payload ) {
        ...
        foreach ( $sanitized as $key => $value ) {
            ...
            if ( 'accessories' === $key && is_array( $value ) ) {
                $existing_accessories = array();
                if ( isset( $data['accessories'] ) && is_array( $data['accessories'] ) ) {
                    $existing_accessories = $data['accessories'];
                }
                foreach ( $value as $acc_key => $acc_value ) {
                    if ( is_null( $acc_value ) ) {
                        unset( $existing_accessories[ $acc_key ] );
                    } else {
                        $existing_accessories[ $acc_key ] = $acc_value;
                    }
                }
                if ( ! empty( $existing_accessories ) ) {
                    $data['accessories'] = $existing_accessories;
                } else {
    ...
    protected function sanitize_submission_payload( $payload ) {
        ...
        if ( array_key_exists( 'address_zip', $clean ) && null !== $clean['address_zip'] ) {
            $clean['address_zip'] = substr( preg_replace( '/[^0-9]/', '', $clean['address_zip'] ), 0, 6 );
        }

        if ( isset( $payload['extras_text'] ) ) {
            $extras = sanitize_textarea_field( $payload['extras_text'] );
            $clean['extras_text'] = '' === trim( $extras ) ? null : $extras;
        }
        ...
        if ( isset( $clean['year_built'] ) ) {
            if ( $clean['year_built'] < 1900 || $clean['year_built'] > $current_year ) {
                unset( $clean['year_built'] );
            }
        }

        if ( empty( $clean['has_renovation'] ) ) {
            $clean['year_renovated'] = null;
            $clean['has_renovation'] = 0;
        } elseif ( isset( $clean['year_renovated'] ) ) {
            if ( $clean['year_renovated'] < 1900 ) {
                $clean['year_renovated'] = 1900;
            }
            if ( isset( $clean['year_built'] ) ) {
                $minimum = $clean['year_built'] + 1;
                if ( $minimum > $current_year ) {
                    $clean['has_renovation'] = 0;
                    $clean['year_renovated'] = null;
                } else {
                    if ( $clean['year_renovated'] <= $clean['year_built'] ) {
                        $clean['year_renovated'] = $minimum;
                    }
                    if ( $clean['year_renovated'] < $minimum ) {
                        $clean['year_renovated'] = $minimum;
                    }
                    if ( $clean['year_renovated'] > $current_year ) {
                        $clean['year_renovated'] = $current_year;
                    }
                }
            } else {
                if ( $clean['year_renovated'] > $current_year ) {
    ...
    public function ajax_upload_photo() {
        ...
        if ( ! empty( $file['error'] ) || empty( $file['tmp_name'] ) ) {
            wp_send_json_error( array( 'message' => __( 'Nahrávanie zlyhalo. Skúste iný súbor.', 'gv-valuation' ) ) );
        }
