(function ($) {
    ...
    function measureSteps($root) {
        var state = readySteps($root);
        var maxHeight = state.measuredHeight || 0;
        $root.find('.gvval-step').each(function () {
            var $step = $(this);
            var isActive = $step.hasClass('gvval-active');
            var previousStyle = $step.attr('style');
            if (!isActive) {
                $step.css({
                    display: 'block',
                    position: 'relative',
            ...
            if (!isActive) {
                if (previousStyle) {
                    $step.attr('style', previousStyle);
                } else {
                    $step.removeAttr('style');
                }
            } else {
                $step.css({ height: '', minHeight: '' });
            }
        });
        if (maxHeight) {
            state.measuredHeight = maxHeight + 40;
        }
    }

    function applyViewportDimensions($root) {
        var state = readySteps($root);
        ...
        if (viewportW > 1200) {
            ...
            widthTarget = Math.min(740, viewportW - 320);
        } else if (viewportW > 820) {
            widthTarget = Math.min(700, viewportW - 220);
        } else if (viewportW > 640) {
            widthTarget = Math.min(660, viewportW - 140);
        } else {
            widthTarget = viewportW - 36;
        }
        widthTarget = Math.max(320, Math.min(widthTarget, 760));

        var heightTarget;
        if (viewportH) {
            var base = Math.max(520, Math.min(viewportH - 200, 680));
            if (viewportH < 680) {
                base = Math.max(480, viewportH - 140);
            }
            heightTarget = base;
        } else {
            heightTarget = 560;
        }
        if (state.measuredHeight) {
            var maxAvailable = viewportH ? Math.max(480, viewportH - 120) : state.measuredHeight;
            heightTarget = Math.max(heightTarget, Math.min(state.measuredHeight, maxAvailable));
            heightTarget = Math.min(heightTarget, 720);
        }

        $root.css('--gv-wrapper-width', widthTarget + 'px');
        $root.css('--gv-step-min-h', heightTarget + 'px');
    }
    ...
    function updateProgress($root) {
        ...
        if ($current.length) {
            var currentHeight = $current[0].scrollHeight;
            if (currentHeight) {
                state.measuredHeight = Math.max(state.measuredHeight || 0, currentHeight + 40);
            }
    ...
    function collectFormData($root) {
        ...
        var zipValue = $.trim($form.find('#address_zip').val()).replace(/[^0-9]/g, '');
        if (zipValue.length > 6) {
            zipValue = zipValue.substring(0, 6);
        }
        data.address_zip = zipValue;
        ...
        var builtYearRaw = parseInt($form.find('#year_built').val(), 10) || '';
        if (builtYearRaw && (builtYearRaw < 1900 || builtYearRaw > currentYear)) {
            builtYearRaw = '';
            $form.find('#year_built').val('');
        }
        data.year_built = builtYearRaw;
        data.has_renovation = $form.find('#has_renovation').is(':checked') ? 1 : 0;
        var renovated = '';
        if (data.has_renovation) {
            var renovationInput = parseInt($form.find('#year_renovated').val(), 10) || '';
            if (renovationInput) {
                if (renovationInput > currentYear) {
                    renovationInput = currentYear;
                    $form.find('#year_renovated').val(renovationInput);
                }
                if (builtYearRaw) {
                    var minAllowed = builtYearRaw + 1;
                    if (renovationInput <= builtYearRaw) {
                        if (minAllowed <= currentYear) {
                            renovationInput = minAllowed;
                            $form.find('#year_renovated').val(renovationInput);
                        } else {
                            renovationInput = '';
                            $form.find('#year_renovated').val('');
                        }
                    }
                    if (renovationInput && renovationInput < minAllowed) {
                        renovationInput = minAllowed;
                        $form.find('#year_renovated').val(renovationInput);
                    }
                }
                if (renovationInput && renovationInput < 1900) {
                    renovationInput = 1900;
                    $form.find('#year_renovated').val(renovationInput);
                }
                renovated = renovationInput || '';
            }
        }
        if (!renovated) {
            data.has_renovation = 0;
        }
    ...
    function refreshRenovationYears($root) {
        ...
        var $checkbox = $root.find('#has_renovation');
        var currentYear = new Date().getFullYear();
        var hasRange = false;
        if (builtYear) {
            var minYear = Math.max(1900, builtYear + 1);
            if (minYear <= currentYear) {
                hasRange = true;
                populateYears($renovated, { min: minYear, max: currentYear });
            } else {
                populateYears($renovated, { min: currentYear, max: currentYear });
            }
        } else {
        ...
        if (!hasRange) {
            ...
            $checkbox.closest('.gvval-checkbox').addClass('is-disabled');
        } else {
            $checkbox.prop('disabled', false);
            $checkbox.removeAttr('aria-disabled');
            $checkbox.closest('.gvval-checkbox').removeClass('is-disabled');
        }
    ...
    function applyDataToForm($root, data) {
        ...
        if (parseInt(data.has_renovation, 10)) {
            ...
            var targetYear = parseInt(data.year_renovated, 10);
            if (targetYear && $form.find('#year_renovated option[value="' + targetYear + '"]').length) {
                $form.find('#year_renovated').val(targetYear);
            } else if (targetYear && data.year_built) {
                var fallbackYear = parseInt(data.year_built, 10) + 1;
                if ($form.find('#year_renovated option[value="' + fallbackYear + '"]').length) {
                    $form.find('#year_renovated').val(fallbackYear);
                }
            }
        }
        if (data.heating) {
    ...
    function validateStep($root) {
        ...
        if (step === 8 && data.has_renovation) {
            ...
            if (data.year_built && data.year_renovated <= data.year_built) {
                return false;
            }
        }
        if (step === 9) {
    ...
    function bindEvents($root) {
        ...
        $root.find('#year_renovated').on('change', function () {
            ...
            if (built && val && val <= built) {
                var nextYear = built + 1;
                if (nextYear <= current) {
                    $(this).val(nextYear);
                } else {
                    $(this).val('');
                    $root.find('#has_renovation').prop('checked', false);
                    $root.find('[data-toggle="has_renovation"]').attr('hidden', 'hidden');
                }
            }
            if (val && val > current) {
    ...
    var handleResize = debounce(function () {
        $('.gvval-wrapper').each(function () {
            var $wrapper = $(this);
            measureSteps($wrapper);
            applyViewportDimensions($wrapper);
        });
    }, 120);
